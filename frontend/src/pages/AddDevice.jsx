...
export default function AddDevice() {
  ...
  const [form, setForm] = useState({
    name: "", type: "thermostat", description: "",
    room: "", category: "",
    status: "off", battery: 100,
    value: 0, target_value: 0, brand: "",
  ...
  return (
    <main className="container" id="main">
      <form className="form form-wide" onSubmit={submit}>
        ...
        <label htmlFor="st">Horaire de début (optionnel)</label>
        <input id="st" type="time" value={form.start_time}
               onChange={(e) => set("start_time", e.target.value)} />

        <label htmlFor="et">Horaire de fin (optionnel)</label>
        <input id="et" type="time" value={form.end_time}
               onChange={(e) => set("end_time", e.target.value)} />

        <label htmlFor="lm">Dernière maintenance (optionnel)</label>
